...
----------------------------GET BITCOIN PRICE--------------------------------------------
   procedure Get_Bitcoin_Price is
      Python_Script : constant String  := "python bitcoin_price_app.py";
      Command       : constant String  := Python_Script & " 2>&1";
      Output        : Unbounded_String := Null_Unbounded_String;
      Line          : String (1 .. 200);
      Last          : Natural;
      Price         : String (1 .. 50) := (others => ' ');
      Block_Height  : String (1 .. 50) := (others => ' ');
      Timestamp     : String (1 .. 50) := (others => ' ');
      Segment       : Natural          := 1;
   ...
   begin
      ...
      begin
         Ada.Text_IO.Open (File, Ada.Text_IO.In_File, Command);
         while not Ada.Text_IO.End_Of_File (File) loop
            Ada.Text_IO.Get_Line (File, Line, Last);
            Output :=
              Output & To_Unbounded_String (Line (1 .. Last)) & ASCII.LF;
         end loop;
         Ada.Text_IO.Close (File);
      end;

      -- Print the raw output for debugging
      Put_Line ("Raw Output from Python Script:");
      Put_Line (To_String (Output));

      -- Check if the output is not empty
      if Output /= Null_Unbounded_String then
         declare
            Parsed_Output : constant String := To_String (Output);
            J             : Natural;
         begin
            -- Parse the output assuming format: "price,block_height,timestamp"
            for J in Parsed_Output'Range loop
               case Parsed_Output (J) is
                  when ',' | ASCII.LF =>
                     Segment    := Segment + 1;
                     Char_Index := 1;
                  when others =>
                     case Segment is
                        when 1 =>
                           if Char_Index <= Price'Length then
                              Price (Char_Index) := Parsed_Output (J);
                              Char_Index         := Char_Index + 1;
                           end if;
                        when 2 =>
                           if Char_Index <= Block_Height'Length then
                              Block_Height (Char_Index) := Parsed_Output (J);
                              Char_Index                := Char_Index + 1;
                           end if;
                        when 3 =>
                           if Char_Index <= Timestamp'Length then
                              Timestamp (Char_Index) := Parsed_Output (J);
                              Char_Index             := Char_Index + 1;
                           end if;
                        when others =>
                           null;
            ...
            -- Trim and display the parsed values
            Put_Line ("Bitcoin Price (USD): " & Price (1 .. Char_Index - 1));
            Put_Line ("Block Height: " & Block_Height (1 .. Char_Index - 1));
            Put_Line ("Timestamp (UTC): " & Timestamp (1 .. Char_Index - 1));
         end;
      else
         Put_Line ("Error: Unable to fetch Bitcoin information.");
      end if;
...
begin
   ...
   Put_Line
     ("------------------------------------------------------------------");
   Put_Line ("Enter name for first party:");
   declare
   ...
   Put_Line ("Enter Public Key (bech32) for participant 1:");
   declare
   ...
   Put_Line ("Enter Public Key (bech32) for participant 2:");
   declare
      Input_PubKey : String (1 .. 66);
      Last_PubKey  : Natural;
   begin
